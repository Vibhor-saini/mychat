    ...
    public function setReceiverId($id) 
{
    ...
    public function setOnlineUsers($users) { $this->onlineUsers = collect($users)->pluck('id')->toArray(); $this->dispatch('online-users-updated', users: array_values($this->onlineUsers)); }
    public function userJoined($user) { if (!in_array($user['id'], $this->onlineUsers)) { $this->onlineUsers[] = $user['id']; $this->dispatch('online-users-updated', users: array_values($this->onlineUsers)); } }
    public function userLeft($user) { $this->onlineUsers = array_diff($this->onlineUsers, [$user['id']]); $this->dispatch('online-users-updated', users: array_values($this->onlineUsers)); }
